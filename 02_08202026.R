...
density_data        <- read.csv("assembled/parr_density_ha.csv")
catchment_data      <- read.csv("explanatory/site_watershed_architecture.csv")
landcover_data_all  <- read.csv("explanatory/sample_lulc_proportions.csv")      # this is the original landcover data
landcover_data      <- landcover_data_all %>%
  select(where(~ !is.numeric(.) || sum(., na.rm = TRUE > 0)))

site_data <- read.csv("meta/site_key.csv") %>% select(-Old) %>%
  left_join(read.csv("meta/sites.csv") %>% select(-Name), by = "Ptagis_ID") %>%
  full_join(read.csv("explanatory/stream_morphology.csv"), by = "Ptagis_ID") %>%
  relocate(Ptagis_ID)
temp_data       <- read.csv("explanatory/sample_modeled_temp_C.csv")
meta_data       <- read.csv("meta/idaho_wild_fish_metadata_2026_06_24.csv")

model_df <- density_data %>%                                    
  left_join(catchment_data, by = "Ptagis_ID") %>%              
  left_join(landcover_data, by = c("Ptagis_ID" = "Ptagis_ID", "Cohort_Year" = "Year")) %>%    # matching the landcover_data "Year" to density_data "Cohort_Year"
  left_join(temp_data, by = c("Ptagis_ID" = "Ptagis_ID", "Cohort_Year" = "Year")) %>%           
  mutate(
...
# ==============================================================================
# 01-b. LOREN'S MODEL FRAME (parr_adj_nona)
#   - joins on COHORT year (not sample year)
#   - filters to electrofishing ("Shock") samples only
#   - log-transforms skewed variables, drops NAs, standardizes
# ==============================================================================

library(janitor)   # clean_names()
library(car)       # qqPlot()

# Figures out how much to add to a variable for log transformations
logadd <- function(x) {
  10^floor(log10(min(x[x > 0], na.rm = TRUE)))
}

# Acquires the additional explanatory variables
density_data   <- read.csv("assembled/parr_density_ha.csv")
catchment_data <- read.csv("explanatory/site_watershed_architecture.csv")
...
# Calculates a temperature range metric (thermal variability)
temp_data$annual_range <- temp_data$Q95 - temp_data$Q05

# Sets the parr density year marker to cohort year
density_data$Year <- density_data$Cohort_Year

# Merges everything together
parr_merge <- density_data[, c("Ptagis_ID", "Year", "prec_parr_ha",
                               "cohort_parr_ha")] %>%
  left_join(catchment_data[, c("Ptagis_ID", "Link", "Shape", "CLink")]) %>%
  left_join(stream_morph) %>%
  left_join(hydro[, c("Ptagis_ID", "Year", "median_annual_Q", "peak_date")]) %>%
  left_join(velocity) %>%
  left_join(temp_data[, c("Ptagis_ID", "Year", "Mean", "annual_range")]) %>%
  left_join(landcover_data[, c("Year", "Ptagis_ID", "NLCD_42", "NLCD_52",
                               "NLCD_71")]) %>%
...
# Standardizes all variables (mean 0, sd 1)
...
flow_files <-list.files(path = "nwm_3_0_WR_17", 
...
streams  <- st_read("spatial", "Salmon_streams_5070.shp")
sites_sf <- st_read("spatial", "Sites_5070.shp")
...
salmon_net <- line2network(path  = "spatial",
...
adult_jack_ratio <- read.csv("explanatory", "adult_jack_ratio.csv")
...
fish_tag <- read.csv("assembled/idaho_wild_fish_tag_data_2026_06_24.csv")
...
# ==============================================================================
# 11. HYPOTHESES (H1-H6)
# H1–H4: single/multi-predictor OLS on the log response, with diagnostics + a scatter. 
# fit_report() cuts the repetition. 
# H6: Mantel spatial test.

library(tidyr)

# helper: fit an lm, print summary + 4 diagnostic plots, draw a scatter
fit_report <- function(formula, title, xvar = NULL, color = "steelblue4",
                       data = parr_adj_nona) {
  m <- lm(formula, data = data)
  print(summary(m))
  op <- par(mfrow = c(2, 2)); plot(m); par(op)
  if (!is.null(xvar)) {
    p <- ggplot(data, aes(x = .data[[xvar]], y = prec_parr_ha)) +
      geom_point(alpha = 0.6, color = color) +
      geom_smooth(method = "lm", color = "black", se = TRUE) +
      theme_minimal() +
      labs(title = title, x = xvar,
           y = "Precocious parr density (log, standardized)")
    print(p)
  }
  invisible(m)
}

# H1. WATERFLOW — density ~ velocity (replaced the old slop proxy)
flow_model <- fit_report(prec_parr_ha ~ v_median,
                         "H1: Water velocity vs Parr Density", 
                         xvar = "v_median", color = "blue")

# H2-2. CROSS SECTION — width-depth ratio + stream sinuosity (replaced slope + TDA proxy)
confinement_model <- fit_report(prec_parr_ha ~ width_depth_ratio + 
                                  stream_sinuosity,
                                "H2-2: Channel cross-section vs Parr Density")

# H3-1. CANOPY — density ~ evergreen forest
forest_model <- fit_report(prec_parr_ha ~ nlcd_42,
                           "H3-1: Evergreen Canopy vs Parr Density", xvar = "nlcd_42",
                           color = "darkgreen")

# H3-2. FIRE (static cover) - shrub + grassland proportion
parr_adj_nona$fire_proxy <- parr_adj_nona$nlcd_52 + parr_adj_nona$nlcd_71
fire_model <- fit_report(prec_parr_ha ~ fire_proxy,
                         "H3-2: Post-Disturbance Cover vs Parr Density", xvar = "fire_proxy",
                         color = "red")

# H3-3. FIRE (rate of change) - recent disturbance rather than static cover
# NOTE: the 2-year window straddles the SAMPLE year; join on COHORT year.
lulc_change <- read.csv("explanatory/site_lulc_change_2y.csv") %>%
  separate(Sample_ID, into = c("Code", "Year"), sep = "-",
           remove = FALSE) %>%
  mutate(Year = as.integer(Year)) %>%
  left_join(site_key, by = "Code")

parr_merge <- parr_merge %>%
  left_join(lulc_change[, c("Ptagis_ID", "Year", "forest_rate",
                            "shrub_rate", "grass_rate")],
            by = c("ptagis_id" = "Ptagis_ID", "year" = "Year"))

fire_rate_model <- fit_report(prec_parr_ha ~ forest_rate + shrub_rate + grass_rate,
  "H3-3: Land Cover Change Rate vs Parr Density")


# H4. SEDIMENT & LWD — multiple regression
sediment_lwd_model <- fit_report(
  prec_parr_ha ~ median_depth + width_depth_ratio + stream_slope,
  "H4: Channel form vs Parr Density")

# H5. COMMUNITY NUTRIENT PROXY: non-Chinook:Chinook ratio (section 15)
# (used to be CHLOROPHYLL A — data-sourcing only, not yet modeled;https://www.waterqualitydata.us/  (National Water Quality Monitoring Council)

# H6. RIVER DISTANCE — do sites close by river have similar densities?
site_density <- parr_adj_nona %>%
  group_by(ptagis_id) %>%
  summarise(mean_parr = mean(prec_parr_ha, na.rm = TRUE)) %>%
  ungroup() %>%
  slice(match(rownames(river_dmat), ptagis_id))

density_dmat <- as.matrix(dist(site_density$mean_parr))
rownames(density_dmat) <- colnames(density_dmat) <- site_density$ptagis_id
...
# H7. COHORT DENSITY - does precocious parr density track cohort density?
density_model <- fit_report(prec_parr_ha ~ cohort_parr_ha,
                            "H7: Cohort Density vs Precocious Parr Density",
                            xvar = "cohort_parr_ha", color = "darkorange3")
# ==============================================================================
...
# ==============================================================================
# 13. POST-LASSO MODEL REFIT

library(lme4)
library(car)

parr_refit <- lmer(prec_parr_ha ~ cohort_parr_ha + width_depth_ratio + nlcd_52 +
                     stream_sinuosity + nlcd_71 + median_depth + link +
                     (1 | ptagis_id),
                   data = parr_adj_nona)

summary(parr_refit)
vif(parr_refit)
png("analyses/refit_residuals.png", width = 6.5, height = 3.5, units = "in",
    res = 300)
par(mfrow = c(1, 2))
plot(fitted(parr_refit), resid(parr_refit), xlab = "Fitted values",
     ylab = "Residuals", pch = 19, col = rgb(0, 0, 0, 0.3))
abline(h = 0, lty = 3)
qqPlot(resid(parr_refit), ylab = "Residuals")
dev.off()



# ==============================================================================
# 14. VARIANCE EXPLAINED (marginal / conditional R2)
library(performance)

r2(parr_refit)

dir.create("data")
write.csv(parr_adj_nona, "data/parr_adj_noNA.csv", row.names = FALSE)
head(parr_adj_nona)

sink("analyses/refit_r2.txt")
cat("Post-LASSO Refit R-squared (Nakagawa method)\n\n")
print(r2(parr_refit))
sink()
...
#===============================================================================
# 15. NUTRIENT PROXY: COMMUNITY COMPOSITION RATIO
# Uses non-Chinook:Chinook ratio as a proxy for productivity/nutrients.
# Rationale: our sites sit on the rising limb of the productivity-diversity curve, so a more diverse assemblage indicates a more productive stream.
# Source: Stearman et al. 2025, DOI 10.5281/zenodo.17209072

library(dplyr)

# Acquires the community and sample event data
community <- read.csv("explanatory/community_data_1992_2024.csv", check.names = FALSE)
events    <- read.csv("explanatory/sample_events_1992_2024.csv", check.names = FALSE)
zen_sites <- read.csv("explanatory/sites.csv")

# Crosswalk from the Zenodo 2-letter codes to our Ptagis_IDs
site_key <- data.frame(
  Code = c("BV","CA","CH","CL","EL","HE","LA","LO","MA","SE","SF","SU",
           "VA","WC","BL","BU"),
  Ptagis_ID = c("BEARVC","CAMASC","CAPEHC","CHAMBC","ELKC","HERDC",
                "LAKEC","LOONC","MARSHC","SECESR","SALRSF","SULFUC","VALEYC",
                "CHAMWF","BIGC","BIG2C"))

# Identifies the Chinook column and the taxa columns
chinook_col <- "Oncorhynchus tshawytscha"
taxa_cols   <- setdiff(names(community), "Sample_ID")

# Calculates the ratio and taxa richness per sample event
comm_ratio <- community %>%
  mutate(
    chinook       = .data[[chinook_col]],
    non_chinook   = rowSums(across(all_of(setdiff(taxa_cols,
                                                  chinook_col))), na.rm = TRUE),
    nonchin_ratio = non_chinook / chinook,
    taxa_richness = rowSums(across(all_of(taxa_cols)) > 0, na.rm = TRUE))
