...
density_data    <- read.csv(here("Precocious_Chinook_Parr_IWF","assembled", "precocious_parr_m2.csv"))
catchment_data  <- read.csv(here("Precocious_Chinook_Parr_IWF","explanatory", "site_watershed_architecture.csv"))
landcover_data  <- read.csv(here("Precocious_Chinook_Parr_IWF","explanatory", "sample_lulc_proportions.csv"))

model_df <- density_data %>%                                    # model_df = master file; "df" = data frame (rows and columns)
  left_join(catchment_data, by = "Ptagis_ID") %>%               # join on site-only (so we are not thinking the changes of the catchment)
  # for the above line, Q: whe/what year did watershed data collected?
  left_join(landcover_data, by = c("Ptagis_ID", "Year"))        # join on site + year
...
#     ~ = "predicted by"
#--------------------------------------------------------------------------
flow_model <- lm(prec_parr_m2 ~ Slope, data = model_df)    
...
ggplot(model_df, aes(x = Slope, y = prec_parr_m2)) + 
...
# #--------------------------------------------------------------------------
# basin_model <- lm(prec_parr_m2 ~ Shape, data = model_df)   # !!shouldn't use Shape to talk about cross-section, since Shape is about watershed/basin
# 
# #--------------------------------------------------------------------------
# # 2. Result and visualization
# #--------------------------------------------------------------------------
...
# library(ggplot2)
# 
# ggplot(model_df, aes(x = Shape, y = prec_parr_m2)) + 
...
#--------------------------------------------------------------------------
confinement_model <- lm(prec_parr_m2 ~ scale(Slope) + scale(TDA_km2), data = model_df)
...
# Drainage area
ggplot(model_df, aes(x = TDA_km2, y = prec_parr_m2)) +
...
# Elevation
ggplot(model_df, aes(x = MeanElev, y = prec_parr_m2)) +
...
#--------------------------------------------------------------------------
forest_model <- lm(prec_parr_m2 ~ NLCD_42, data = model_df)
...
ggplot(model_df, aes(x = NLCD_42, y = prec_parr_m2)) + 
...
fire_model <- lm(prec_parr_m2 ~ fire_proxy, data = model_df)
...
ggplot(model_df, aes(x = fire_proxy, y = prec_parr_m2)) +
...
#--------------------------------------------------------------------------
sediment_lwd_model <- lm(prec_parr_m2 ~ sediment_risk + lwd_proxy + Slope,
                         data = model_df)
#--------------------------------------------------------------------------
# 2. Result & visualization
#--------------------------------------------------------------------------
...
ggplot(model_df, aes(x = sediment_risk, y = prec_parr_m2)) +
...
ggplot(model_df, aes(x = lwd_proxy, y = prec_parr_m2)) +
